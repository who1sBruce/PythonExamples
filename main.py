			...
			for tank_player in stage_data.mytanksGroup:
				...
				if pygame.sprite.collide_rect(tank_player, myfood):
					...
					if myfood.kind == 1:
						...
						for each in stage_data.enemytanksGroup:
							each.can_move = False
						pygame.time.set_timer(stage_data.recoverEnemyEvent, 8000, loops=1)
					# 子弹增强
					if myfood.kind == 2:
						sound_data.add_sound.play()
					...
					if myfood.kind == 4:
						...
						for t in stage_data.mytanksGroup:
							t.protected = True
						pygame.time.set_timer(stage_data.noprotectMytankEvent, 8000, loops=1)
					# 坦克升级
					if myfood.kind == 5:
						sound_data.add_sound.play()
		...
		while True:
			...
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					...
					sys.exit()
				# 生成新的敌方坦克
				if event.type == stage_data.genEnemyEvent:
					if stage_data.enemytanks_total > 0:
						if stage_data.enemytanks_now < stage_data.enemytanks_now_max:
